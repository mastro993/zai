...
use budget_contract::{
    ...
    delete_no_content, detail_success, history_success, history_validation_error, list_active_success,
    name_conflict_error, not_found_detail, pause_success, request_delete, request_pause,
    request_resume, request_update, resume_success, revision_conflict_update, seed_budget,
    setup_contract, update_success,
};
...
async fn budget_contract_create_list_detail_and_history_match_across_transports() {
    ...
    compare_http_and_tauri(&http, &tauri, |_| create_success("Monthly spending"), "", "")
        .await;
...
async fn budget_contract_error_operations_match_across_transports() {
    ...
    compare_http_and_tauri(&http, &tauri, |_| name_conflict_error(), &http_id, &tauri_id).await;
