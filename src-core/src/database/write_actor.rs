...
pub struct WriteHandle {
    ...
    // Each job is a boxed closure, and one oneshot sender is used for the reply.
...
impl WriteHandle {
    ...
    {
        // Create an oneshot channel for receiving the result from the actor.
...
pub fn spawn_writer(pool: DbPool) -> WriteHandle {
    ...
    tokio::spawn(async move {
        ...
        while let Some((job, reply_tx)) = rx.recv().await {
            ...
            // Ignore error if the receiver has dropped (e.g., request timed out or was canceled).
