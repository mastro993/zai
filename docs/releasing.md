...
Nightly notes are generated by GitHub since the previous Nightly and include
the full changelog and contributors. Stable requires a non-empty curated
`release-notes/<tag>.md`; the workflow fails before builds when it is missing.
