...
use super::row_mapping::category_from_row;
...
pub(super) fn delete_categories(
    ...
    diesel::update(
        transaction_categories::table.filter(transaction_categories::id.eq_any(&ids_to_delete)),
    )
    ...
    diesel::update(
        transactions::table.filter(transactions::transaction_category_id.eq_any(&ids_to_delete)),
    )
    .set((
